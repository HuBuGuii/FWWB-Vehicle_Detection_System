...
def run(
    data=ROOT / "../datasets/A10_new/images",  # dataset dir
    weights=ROOT / "runs\\train-cls\exp13\weights\\best.pt",  # model.pt path(s)
    ...
    with torch.cuda.amp.autocast(enabled=device.type != "gpu"):
...
def parse_opt():
    ...
    parser.add_argument("--data", type=str, default=ROOT / "../datasets/A10_new/images", help="dataset path")
    parser.add_argument("--weights", nargs="+", type=str, default=ROOT / "runs\\train-cls\exp13\weights\\best.pt", help="model.pt path(s)")
